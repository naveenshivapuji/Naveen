package raghu.teaching.agrregation;

public class StingToCharArray {

	public static void main(String[] args) {

//		char ch[] = {'J','S','P','I','D','E','R'};
//		String s1 = new String(ch);
//		
//		String s2 = new String("QSpider");
//		char c[] = s2.toCharArray();
		
		String s1 = new String("jsp");
		String s2 = new String("qsp");
		String s3 = new String("jsp");
		
		System.out.println(s1);
		System.out.println(s2);
		System.out.println(s3);
		
		System.out.println("---------------------------");
		
		System.out.println(s1.hashCode());
		System.out.println(s2.hashCode());
		System.out.println(s3.hashCode());
		
		System.out.println("----------------------------");
		
		System.out.println(s1.equals(s2));
		System.out.println(s1.equals(s3));//compares the content of the string
		System.out.println(s2.equals(s3));
		
		System.out.println("----------------------------");
		
		System.out.println(s1==s2); 
		System.out.println(s1==s3);//compares address of the string
		System.out.println(s2==s3);

		
		
	}

}
